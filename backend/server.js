...
app.post('/summarize', async (req, res) => {
  ...
  try {
    ...
    let parsed = null;
    try { parsed = JSON.parse(clean); } catch { console.warn('/summarize: non-JSON response'); }
...
app.post('/generate-email', async (req, res) => {
  ...
  try {
    ...
    if (actionItems && actionItems.length > 0) {
      actionItemsText = actionItems.map((item, i) => { let line = `${i + 1}. ${item.task || item.action}`; if (item.owner) line += ` (Owner: ${item.owner})`; if (item.deadline) line += ` (Due: ${item.deadline})`; return line; }).join('\n');
    }
    const completion = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [
        { role: 'system', content: `You are a professional email writer for Indian businesses. Generate a polished follow-up email. Rules:\n1. Write in clear professional English\n2. Subject: "Follow-up: [Meeting Title]"\n3. Include: greeting, summary, action items, next steps, closing\n4. Max 250 words\n5. Format: Subject on first line, blank line, then body\n6. Return ONLY the email` },
        { role: 'user', content: `Title: ${title || 'Meeting'}\nDate: ${date || new Date().toLocaleDateString('en-IN')}\nSummary:\n${summary || transcript.substring(0, 1000)}\nAction Items:\n${actionItemsText || 'None identified'}` }
      ],
      max_tokens: 600,
    });
  ...
  try {
    ...
    if (data.auto_summary) {
      ...
      if (s && s.executive_summary) {
        ...
        const row = (label, value) => val(value) ? `<div style="margin-bottom:10px;"><span style="font-weight:700;color:#555;font-size:11px;text-transform:uppercase;">${label}</span><div style="margin-top:4px;font-size:14px;color:#333;line-height:1.6;">${value}</div></div>` : '';
        const list = (items) => arr(items) ? `<ul style="margin:4px 0 0;padding-left:18px;">${items.map(i=>`<li style="margin-bottom:4px;font-size:13px;color:#333;">${i}</li>`).join('')}</ul>` : '';
        const makeTable = (headers, rows) => { if (!arr(rows)) return ''; const th = headers.map(h=>`<th style="padding:8px 10px;background:#1A56A0;color:#fff;text-align:left;font-size:12px;">${h}</th>`).join(''); const tr = rows.map((r,i)=>`<tr style="background:${i%2===0?'#fff':'#F5F8FC'};">${Object.values(r).map(v=>`<td style="padding:8px 10px;border-bottom:1px solid #E8EEF5;font-size:13px;color:#333;">${v||'Not specified'}</td>`).join('')}</tr>`).join(''); return `<table style="width:100%;border-collapse:collapse;"><thead><tr>${th}</tr></thead><tbody>${tr}</tbody></table>`; };
        ...
        if (arr(s.risks_concerns)) html11 += section('7. Risks', makeTable(['Risk','Mentioned By','Context'], s.risks_concerns.map(r=>({'Risk':r.risk,'Mentioned By':r.mentioned_by,'Context':r.context}))));
        if (arr(s.important_highlights)) html11 += section('8. Highlights', list(s.important_highlights));
        if (arr(s.quotes_worth_retaining)) html11 += section('9. Quotes', s.quotes_worth_retaining.map(q=>`<div style="border-left:4px solid #7B1FA2;background:#F3E5F5;padding:10px 14px;margin-bottom:10px;"><div style="font-size:13px;color:#4A148C;font-style:italic;">"${q.quote}"</div><div style="font-size:11px;color:#7B1FA2;margin-top:4px;">— ${q.speaker}</div></div>`).join(''));
        if (s.one_page_brief) { const b = s.one_page_brief; html11 += section('10. One-Page Brief', `<div style="background:#EFF6FF;border-radius:8px;padding:14px;border:1px solid #BFDBFE;"><table style="width:100%;border-collapse:collapse;">${[['Purpose',b.purpose],['Decisions',b.decisions],['Actions',b.actions],['Risks',b.risks],['Pending',b.pending_items]].filter(([,v])=>val(v)&&v!=='None identified').map(([k,v])=>`<tr><th style="padding:8px 10px;text-align:left;font-size:12px;color:#1A56A0;width:22%;">${k}</th><td style="padding:8px 10px;font-size:13px;color:#333;border-bottom:1px solid #DBEAFE;">${v}</td></tr>`).join('')}</table></div>`); }
...
const transcribeWithSarvam = async (audioFilePath, langCode) => {
  ...
  try {
    const response = await sarvam.speechToText.transcribe({ file_path: audioFilePath, language_code: langCode, model: 'saaras:v3', mode: 'translate', with_diarization: true });
    console.log('[Sarvam] Done. Turns:', response.turns?.length || 0);
    const utterances = (response.turns || []).map((turn, i) => ({ speaker: turn.speaker || `Speaker ${String.fromCharCode(65 + i)}`, text: turn.text || '', englishText: turn.text || '', start: (turn.start_time_ms || 0), end: (turn.end_time_ms || 0), words: [] }));
...
const translateToEnglish = async (text) => {
  try {
    const completion = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [{ role: 'system', content: `You are a translator for Indian languages. Rules:\n1. Translate everything to clean natural English\n2. Keep names, places, company names as-is\n3. If text is already in English — return it as-is\n4. Return ONLY the translated text, nothing else` }, { role: 'user', content: text }],
      max_tokens: 2000,
    });
...
const generateSummary = async (text, mode) => {
  try {
    ...
    const raw = completion.choices[0].message.content.trim();
    const clean = raw.replace(/^```json\s*/i, '').replace(/^```\s*/i, '').replace(/\s*```$/i, '').trim();
    try { JSON.parse(clean); return clean; } catch { return raw; }
  } catch (err) {
...
const extractActionItems = async (text) => {
  try {
    ...
    const completion = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [{ role: 'system', content: `Extract action items. Return ONLY a JSON array:\n[{"task": "...", "owner": "...", "deadline": "..."}]\nIf none, return: []` }, { role: 'user', content: 'Extract action items:\n\n' + truncated }],
      max_tokens: 500,
    });
    const raw = completion.choices[0].message.content.trim();
...
const generateTitle = async (text, detectedLang) => {
  try {
    ...
    const completion = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [{ role: 'system', content: `Generate a SHORT title in English. Format: "[Topic] — [Key Context]". Max 6 words. Return ONLY the title.` }, { role: 'user', content: 'Generate a title:\n\n' + truncated }],
      max_tokens: 30,
    });
...
const detectSpeakerNames = async (utterances) => {
  try {
    ...
    const completion = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [{ role: 'system', content: `Detect speaker names from introductions. Return ONLY a JSON object like {"Speaker A": "Anshul"}. If none found, return {}. Return ONLY the JSON.` }, { role: 'user', content: `Detect names:\n\n${dialogue}` }],
      max_tokens: 200,
    });
    const raw = completion.choices[0].message.content.trim();
    const parsed = JSON.parse(raw);
...
const processTranscript = async (transcript, mode, sarvamResult = null) => {
  ...
  if (sarvamResult && sarvamResult.success) {
    ...
    // ✅ Run all 4 GPT steps in parallel for speed
    const [autoSummary, actionItems, smartTitle, speakerNameMap] = await Promise.all([
      summaryInput ? generateSummary(summaryInput, resolvedMode) : Promise.resolve(null),
      summaryInput ? extractActionItems(summaryInput) : Promise.resolve([]),
      summaryInput ? generateTitle(summaryInput, sarvamResult.language_code) : Promise.resolve(null),
      detectSpeakerNames(sarvamResult.utterances),
    ]);
    console.log('Smart title result:', smartTitle);
    console.log('Detected speaker names:', speakerNameMap);
  ...
  const [autoSummary, actionItems, smartTitle, speakerNameMap] = await Promise.all([
    summaryInput ? generateSummary(summaryInput, resolvedMode) : Promise.resolve(null),
    summaryInput ? extractActionItems(summaryInput) : Promise.resolve([]),
    summaryInput ? generateTitle(summaryInput, detectedLang) : Promise.resolve(null),
    detectSpeakerNames(englishUtterances || utterances),
  ]);
  console.log('Smart title result:', smartTitle);
  console.log('Detected speaker names:', speakerNameMap);
...
app.post('/transcribe-start', upload.single('audio'), async (req, res) => {
  ...
  try {
    ...
    const job = await aai.transcripts.submit({
      audio: uploadUrl, speaker_labels: true, language_detection: true,
      format_text: true, punctuate: true,
...
app.post('/webhook/assemblyai', async (req, res) => {
  try {
    ...
    const { data: existingJob } = await supabase
      .from('transcription_jobs').select('status, result, mode').eq('id', transcript_id).single();
    ...
    // ✅ FIX 1: Error handling + retry on final Supabase update
    // Without this, if update fails silently, status stays 'processing_lock' forever
    ...
    if (updateError) {
      console.error('❌ Supabase update failed (attempt 1):', updateError.message);
      ...
      const { error: retryError } = await supabase
        ...
        .eq('id', transcript_id);
      if (retryError) {
        console.error('❌ Supabase update failed (attempt 2):', retryError.message);
      } else {
        console.log('✅ Supabase update succeeded on retry. Job:', transcript_id);
      }
    }
...
app.get('/transcribe-status/:jobId', async (req, res) => {
  try {
    ...
    if (jobId.startsWith('sarvam_')) {
      return res.json({ success: false, status: 'error', error: 'Sarvam job result not found' });
    }
    ...
    if (transcript.status === 'completed') {
      ...
      // ✅ Error handling on fallback update too
      ...
      if (fallbackError) {
        console.error('❌ Fallback Supabase update failed:', fallbackError.message);
        // Still return the result to the app even if DB save failed
      }
