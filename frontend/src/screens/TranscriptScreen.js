  ...
  const exportMinutesPDF = async () => {
    ...
    try {
      ...
      if (parsed) {
        ...
        summarySection += buildList(parsed.decisions,     '#1A7A4A', '✅', 'Decisions');
      ...
      if (actionItems.length > 0) {
        const rows = actionItems.map((item, i) => `
          <tr style="background:${i % 2 === 0 ? '#FFFFFF' : '#F8FAFC'};">
            <td style="padding:10px 12px;font-weight:700;color:#1A56A0;width:36px;">${i + 1}</td>
            <td style="padding:10px 12px;color:#1E293B;font-weight:500;">${item.task}</td>
            <td style="padding:10px 12px;color:#64748B;width:120px;">${item.owner && item.owner !== 'Unassigned' ? '👤 ' + item.owner : '—'}</td>
            <td style="padding:10px 12px;color:#64748B;width:120px;">${item.deadline && item.deadline !== 'Not mentioned' ? '📅 ' + item.deadline : '—'}</td>
          </tr>`).join('');
        actionSection = `
          <div style="margin-top:28px;">
            <div style="font-size:14px;font-weight:700;color:#E65100;margin-bottom:12px;padding-left:10px;border-left:3px solid #FF9800;">✅ Action Items</div>
            <table style="width:100%;border-collapse:collapse;border-radius:8px;overflow:hidden;border:1px solid #FFE0B2;">
              <thead><tr style="background:#FF9800;">
                <th style="padding:10px 12px;color:#fff;text-align:left;font-size:12px;">#</th>
                <th style="padding:10px 12px;color:#fff;text-align:left;font-size:12px;">Task</th>
                <th style="padding:10px 12px;color:#fff;text-align:left;font-size:12px;width:120px;">Owner</th>
                <th style="padding:10px 12px;color:#fff;text-align:left;font-size:12px;width:120px;">Deadline</th>
              </tr></thead>
              <tbody>${rows}</tbody>
            </table>
          </div>`;
      ...
      if (canShare) {
        await Sharing.shareAsync(uri, { mimeType: 'application/pdf', dialogTitle: 'Share Meeting Minutes', UTI: 'com.adobe.pdf' });
      } else {
        Alert.alert('PDF Created', 'Saved to: ' + uri);
      }
    } catch (err) {
      console.error('Minutes PDF error:', err);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <TouchableOpacity style={styles.copyNotesBtn} onPress={copyAINotes}>
          <Text style={styles.copyNotesBtnIcon}>🤖</Text>
          <Text style={styles.copyNotesBtnText}>Copy AI Notes to Clipboard</Text>
        </TouchableOpacity>
